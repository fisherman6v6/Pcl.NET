...
{
    public class PointCloudXYZITests : IDisposable
    {
        ...
        {
            _cloud = PointCloudXYZI.Load(TestFile);
        }
        ...
        public void Points_ShouldBeAccessible()
        {
            ...
            // Access first point to verify XYZI coordinates are readable
            ...
            Assert.True(firstPoint.Intensity >= 0);
        }
        ...
        public void At_ValidIndices_ShouldReturnPoint()
        {
            ...
            Assert.True(point.Intensity >= 0);
        }

        [Fact]
        public void Concatenate_TwoPointClouds_ShouldCombineCounts()
        {
            var cloud1 = new PointCloudXYZI(2, 2);
            var cloud2 = new PointCloudXYZI(2, 2);

            cloud1.Add(new PointXYZI(1, 1, 1, 50));
            cloud1.Add(new PointXYZI(2, 2, 2, 60));
            cloud2.Add(new PointXYZI(3, 3, 3, 70));
            cloud2.Add(new PointXYZI(4, 4, 4, 80));

            var combined = PointCloudXYZI.Concatenate(cloud1, cloud2);
        ...
        {
            var cloud = new PointCloudXYZI();
            var initialCount = cloud.Count;
            cloud.Add(new PointXYZI(1, 2, 3, 100));
        ...
        public void Constructor_WithDimensions_ShouldCreateEmptyCloud()
        {
            var width = 10;
            var height = 5;
            var cloud = new PointCloudXYZI(width, height);
        ...
        {
            var copy = new PointCloudXYZI(_cloud);
            ...
            for (int i = 0; i < Math.Min(5, _cloud.Count); i++)
            {
                ...
                Assert.Equal(_cloud.Points[i].Intensity, copy.Points[i].Intensity);
            }
        }
        ...
        {
            var cloud1 = new PointCloudXYZI(2, 1);
            var cloud2 = new PointCloudXYZI(2, 1);

            cloud1.Add(new PointXYZI(1, 1, 1, 50));
            cloud2.Add(new PointXYZI(2, 2, 2, 60));
