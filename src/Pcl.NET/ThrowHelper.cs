...
namespace Pcl.NET
{
    internal static class ThrowHelper
    {
        ...
        public static void ThrowArgumentOutOfRangeIfCondition_IndexMustBeNonNegativeException(bool condition, string name)
        {
            if (condition)
            {
                throw new ArgumentOutOfRangeException($"Index was out of range. Must be non-negative. (Parameter '{name}')");
            }
        }
        [DoesNotReturn]
